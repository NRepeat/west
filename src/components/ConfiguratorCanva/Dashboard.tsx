                ...
                <Popover>
                    <PopoverTrigger>
                        <DashboardButton
                            onClick={() => {}}
                            type="pallet"
                            active={wheels.color !== wheels.defaultColor}
                        />
                    </PopoverTrigger>
...
const DashboardCard: FC<DashboardCardProps> = ({ title, children, className }) => {
    return (
        <Card
            className={clsx(
                className,
                'flex flex-col gap-2.5 bg-[#454545] p-2.5 rounded-sm h-full',
            )}
        >
            <CardTitle className="text-center w-full text-white text-2xl">{title}</CardTitle>
            <Separator className="bg-[#5A5A5A]" />
            <CardContent className="flex gap-2.5 p-4">{children}</CardContent>
        </Card>
    );
};
...
export const DashboardButton: FC<{
    ...
    return (
        <Button
            onClick={onClick}
            variant={'default'}
            className={clsx('border-[#99A3A3] border-2 p-6 h-14 ', { 'border-[#26A142]': active })}
        >
            {getIcon(type)}
        </Button>
    );
