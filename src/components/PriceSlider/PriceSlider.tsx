		...
		<Card >
			<CardContent className='h-auto flex flex-col gap-2'>
				...
				<div className='flex w-full box-border p-2 gap-4 justify-between items-center'>
					<FromInput scope={form.scope("min")} label={'From'} islabelvisible={true} placeholder='$ 100' name='min' handleChangeValue={handleChangeMin} />
					<FromInput scope={form.scope("max")} label={'To'} islabelvisible={true} placeholder='$ 1000' name='min' value={values[1]} handleChangeValue={handleChangeMax} />
				</div>
			</CardContent>
		</Card>
	)
