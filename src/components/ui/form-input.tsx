...
import React, { ComponentPropsWithRef, FC, forwardRef, useId } from 'react';
...
type InputType = <Type extends string>(props: MyInputProps<Type>) => React.ReactNode;

const InputImpl = forwardRef<HTMLInputElement, MyInputProps<string>>(
    (
        {
            handleChangeValue,
            className,
        ...
        return (
            <div
                ...
                className={clsx(
                    className,
                    'inline-flex flex-col w-full relative  focus:outline-none focus-visible:outline-none gap-1 rounded-sm',
                )}
            >
                {islabelvisible && (
                ...
                <Input
                    className={clsx(
                        'bg-backgroundContainer text-xl w-full px-2.5 placeholder:text-lg  focus:outline-none focus-visible:outline-none   rounded-sm  placeholder:text-[#999999] border-0 relative',
                        { 'pr-8': search },
                    )}
                    {...field.getInputProps({
                        type,
