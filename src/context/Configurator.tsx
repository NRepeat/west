    ...
    return (
        <ConfiguratorContext.Provider value={{ disks: null }}>
            ...
            >
                                    ...
                                    {showCanvas ? (
                                        ...
                                        >
                                            <Suspense fallback={<Loader />}>
                                                {disks.map((disk) => (
                                                    <Preload key={disk.path} model={disk.path} />
                                                ))}
                                                {children}
                                            </Suspense>
                                        </Canvas>
                ...
                {!fullScreen && (
                    <Canvas
                        camera={{ near: 10.1, far: 1200, position: [54, 53, 50] }}
                        gl={{ antialias: true, pixelRatio: window.devicePixelRatio }}
                    >
                        <Suspense fallback={<Loader />}>
                            {disks.map((disk) => (
                                <Preload model={disk.path} />
                            ))}
                            {children}
                        </Suspense>
                    </Canvas>
...
function Loader() {
    ...
    return (
        <Html fullscreen className="bg-white text-white ">
            <DLoader isResponsive></DLoader>
        </Html>
    );
}
