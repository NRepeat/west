    ...
    immer((set) => ({
        ...
        cameraConfig: {
            ...
            setCameraPosition: (position) =>
                set((state) => {
                    state.cameraConfig.cameraPosition = position;
                }),
            setIsDefaultAnimation: (isDefault) =>
                set((state) => {
                    state.cameraConfig.defaultAnimation = isDefault;
                }),
            timeoutTime: 1400,
        },
        orbitControlBehaviorConfig: {
            orbitControlBehavior: false,
            setOrbitControlBehavior: (is) =>
                set((state) => {
                    state.orbitControlBehaviorConfig.orbitControlBehavior = is;
                }),
        },
        selectedIndex: 0,
        ...
        setAnimationState: (isAnimationStarted: boolean) =>
            set((state) => {
                state.isAnimationStarted = isAnimationStarted;
            }),
    })),
);
